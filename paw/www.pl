...
sub do_cmd_PAWcdef {
    ...
    s/$next_pair_pr_rx//o;
    $params = $2;
    join ('', "<strong>$command $params<\/strong><p>", $_);
}
sub do_cmd_PAWfind {
    local ($_) = @_;
    s/$optional_arg_rx//o;
...
sub do_cmd_PAWfdefii{
    ...
    join ('',"<STRONG>$parameters1<\/strong><\/br>".
             "<STRONG>$parameters2<\/strong><\/br>");
}
sub do_cmd_IPAWCC {
    local($_) = @_;
    join('',"<TT>\/PAWC\/<\/TT>",$_);
...
sub do_cmd_epsfig {
    ...
    s/$next_pair_pr_rx//o;
    $nb = $2;
    join ('', "<P><TT>EPS Figure $nb<\/TT><p>", $_);
}
sub do_cmd_epsfbox {
    local ($_) = @_;
    s/$next_pair_pr_rx//o;
...
sub do_cmd_pawexfig {
    ...
    s/$next_pair_pr_rx//o;
    $nb = $2;
    join ('', "<P><TT>EPS Figure pawexa $nb<\/TT><p>", $_);
}
sub do_cmd_pawtutfig {
    local ($_) = @_;
    s/$next_pair_pr_rx//o;
    $nb = $2;
    join ('', "<P><TT>EPS Figure pawtut $nb<\/TT><p>", $_);
}
sub do_cmd_lcb {
    local ($_) = @_;
    join ('', '\{', $_);
