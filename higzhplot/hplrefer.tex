...
%  Last Mod.: Wed Feb 15 13:22:58 MET 1995                                 %
%                                                                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
set in \Rind{HPLSET} with parameter \Ssind{BTYP}, and to the fill area colour
index set in \Rind{HPLSET} with parameter \Ssind{BCOL}. The contour is always
drawn.
\Pdesc
\begin{DLtt}{12345}
...
\item The line type can be changed with parameter \Ssind{DMOD} of \Rind{HPLSET}.
...
\item[IWTYP] Workstation type.See {\bf appendix B} for the list of valid
             workstation types. If \Lit{IWTYP=0} no
...
the parameter \Ssind{CSIZ} using routine \Rind{HPLSET}, which defines also the 
symbol size.
...
\begin{longtable}{|p{.11\textwidth}|p{.15\textwidth}|p{.7\textwidth}|}
...
\item If the options '\Oind{A3}' or '\Oind{A4}' are called, windowing is turned
      off (i.e. a call to \Lit{HPLZON(1,1,1,' ')} is performed). It is recommended 
...
\item If option '\Oind{ZFL1}' is selected only the last created picture is kept
      in memory.
\item With the '\Oind{BAR}' option parameter \Ssind{HTYP} of \Rind{HPLSET} can 
...
\item One can force a particular type of line style by calling routine 
      \Rind{HPLSET} with parameter \Ssind{DMOD}, e.g. 
...
\item The fill area interior style and style index can be changed with parameter
      \Ssind{HTYP} in \Rind{HPLSET} (this parameter has to be set to draw a 
...
\item The colour (contour or surface) of the histogram can be changed with 
      parameter \Ssind{HCOL} in \Rind{HPLSET}.
...
\item \Ssind{HMAX} is given in percent (default value is \Lit{90\%}).
\item The values given to the parameters \Ssind{PTYP}, \Ssind{BTYP} and 
      \Ssind{HTYP} are fill area interior style. These parameters are 
      ...
      \ref{FILL-IS}. The parameters \Ssind{PCOL}, \Ssind{BCOL}, \Ssind{HCOL} are 
      equivalent to \Ssind{PTYP}, \Ssind{BTYP}, \Ssind{HTYP}, respectively, but 
      instead of changing the hatch style, they change the colour of the same 
      areas.
\item If \Ssind{PCOL}, \Ssind{BCOL}, \Ssind{HCOL} are between {\tt1} and {\tt99},
      ...
      corresponding fill area colour index. For \Ssind{PCOL}, \Ssind{BCOL} or
      \Ssind{HCOL} the corresponding value of the Fill Area Interior Style 
      (for \Ssind{PTYP}, \Ssind{BTYP}, \Ssind{HTYP}) is automatically set to 1 
      (solid).
...
\index{colour}
\item \Ssind{TFON}, \Ssind{GFON}, \Ssind{VFON} and \Ssind{LFON} must be set 
...
\item \Ssind{*SIZ}, \Ssind{*TYP}, \Ssind{*COL}, \Ssind{*WID} and \Ssind{*FON} define 
...
\item The label sets defined by the routine \Rind{HPLABL} can be used for axes 
      on all plots produced by \HPLOT{} via the \Ssind{NDVX}, \Ssind{NDVY} and 
      ...
      according the number of zones (see \Lit{HPLZON}) i.e : 
      ...
      \newpage
      \begin{Fighere}
      \begin{center}\includegraphics[width=16cm]{ndvx.eps}\end{center}
      \caption{Example of labelling for horizontal axes}
      \label{fig:LABNDVX}
      \end{Fighere}
 
      \begin{Fighere}
      \begin{center}\includegraphics[width=16cm]{ndvy.eps}\end{center}
...
\begin{figure}[p]
\begin{center}\includegraphics[width=.9\linewidth]{hplset.eps}\end{center}
\caption{A graphical view of the \protect\Rind{HPLSET} parameters.}
\label{fig:HPLSET}
\end{figure}

\begin{figure}[p]
\begin{center}\includegraphics{btyp.eps}\end{center}
\caption[The {\tt HPLSET} parameters {\tt PTYP}, {\tt BTYP} and {\tt HTYP}]%
        {The \Rind{HPLSET} parameters \Ssind{PTYP}, \Ssind{BTYP}, \Ssind{HTYP}}
\label{TYPE}
\end{figure}
\clearpage
...
\item Note that boldface characters may be simulated by with the 
      \Ssind{PASS} and \Ssind{CSHI} attributes of \Rind{HPLSET}. The meaning of these 
      attributes is the following: Every stroke used to display the character is
      repeated \Ssind{PASS} times, at a distance (in percentage of the character 
      height) given by \Ssind{CSHI}.
...
\item[USIZE] Size of the symbol (in cm). If \Lit{USIZE = 0.} then the size of 
             ...
             which can be changed with the parameter \Ssind{CSIZ} of 
             \Rind{HPLSET}.
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotscat.eps}\end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotbox.eps} \end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotarr.eps} \end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotcont.eps} \end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotcol.eps} \end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplottext.eps} \end{center}
...
\end{center}
\bigskip
\begin{center} \includegraphics{hplotchar.eps} \end{center}
...
options \Ssind{XWIN} and \Ssind{YWIN}.
...
parameter \Ssind{HMAX} in \Rind{HPLSET} (default value for \Ssind{HMAX} is 0.9).
