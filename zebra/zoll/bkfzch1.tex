...
\Filename{H2-FZ-Data-Structure-Representation}
...
\Filename{H2-FZ-Outline-Usage-for-disk-tape}
...
On normal completion \Rind{FZIN} returns IQUEST(1)=0;
a positive value indicates an exception,
like Start-of-run or End-of-data;
a negative value signals trouble.
IQUEST(1) \textbf{must} be tested after every call to \Rind{FZIN}.
...
he can indicate this to \Rind{FZOUT} with the 'DI' option,
...
on to \Rind{FZFILE} via IQUEST(1).
...
by means other than calling \Rind{FZIN}: giving the S option
...
set by MZEBRA.
By giving the Q (quiet) option with \Rind{FZFILE} the level is set to -2.
...
\Filename{H2-FZHOOK-connect-user-routine-for-medium-Channel}
...
\Filename{H2-FZLIMI-limit-size-output-file}
...
\Filename{H2-FZODAT-storing-recovering-direct-access-table}
...
If LDAT is non-zero the DaT bank is written and the forward reference
...
\Filename{H2-FZRUN-write-RUN-record}
...
\Filename{H2-FZOUT-write-one-data-structure}
...
\end{verbatim}
   ...
   by calling \Rind{FZIN}FO, see para. 4.01.
...
but exits to ZTELL.
Exceptionally, error returns may be enabled by the P option.
\begin{verbatim}
   IQUEST(1) = -2
...
\Filename{H2-FZIN-read-one-data-structure}
...
at the same time delivering the user-header vector to IUHEAD.
...
\subsection*{Specifications for \Rind{FZIN}}

\Rind{FZIN} returns the read status, either normal or error, in IQUEST;
...
\Filename{H2-FZIN-read-one-data-structure-by-segments}
...
the store is selected by the parameter IXDIV to \Rind{FZIN}).
To cause this segment to be ignored IQSEGD(J) = --1 should be set.
IQSEGD(J) containing zero directs this segment into the
'default' division selected by the parameter IXDIV to \Rind{FZIN}.
(The vector IQSEGD is preset to zero by \Rind{FZIN}.)
...
\Filename{H2-FZINXT-reset-read-point-on-direct-access-file}
...
\Filename{H2-FZCOPY-copy-one-data-structure-from-input-to-output}
...
\end{verbatim}
Details about the error occurred are stored in IQUEST(11) ff.
as described in the Zebra Reference Manual, book DIA for \Rind{FZIN}.
...
\Filename{H2-FZENDO-output-file-termination}
...
\Filename{H2-FZENDI-input-file-termination}
...
To obtain a particular data-structure one has to call \Rind{FZIN}XT specifying
...
including into it IQUEST(5) and (6) returned from \Rind{FZOUT}.
...
1) call \Rind{FZFILE} giving CHOPT = 'D' or 'LD' to permit random access.
...
This resets the read point, calling \Rind{FZIN} (LUN,...) will then
...
\begin{itemize}
 \item[--]  the 'stream' must be initialized with the M option in \Rind{FZFILE},
 \item[--]  the options I or O set this stream for input or output
               (it cannot be used for both concurrently),
 \item[--]   the parameter LUN to \Rind{FZFILE} identifies the stream,
...
\Rind{FZOUT} returns in IQUEST(9) the number of useful machine words
...
a little care,
as explained in the note about LREC of para. 1.04 for \Rind{FZFILE},
and also below.
...
Using the information of chapter 3,
...
and \Rind{FZOUT} will return IQUEST(9)=NWUSE.
...
machine word,
and \Rind{FZOUT} will return IQUEST(9)=(NWUSE+1)/2.
...
\Rind{FZOUT} returns again IQUEST(9)=NWUSE.
...
\begin{itemize}
\item the 'stream' must be initialized with the C option in \Rind{FZFILE},
\item the options I or O set this stream for input or output
      (it cannot be used for both concurrently),
\item the parameter LUN to \Rind{FZFILE} identifies the stream,
\item the N option selects the native data format,
...
\fbox{\Lit{SUBROUTINE UserSR (IBUF,IOWAY)}}
\begin{verbatim}
...
UserSR is supposed to dispatch the buffer IBUF,
and return zero in IQUEST(1).
(At least for the time being, a non-zero status code in IQUEST(1)
is ignored.)

\Filename{H2-User-marking-of-data-structures-for-FZOUT}
...
Note that we have used the anti-selection option (--) of \Rind{MZMARK}
...
\Filename{H2-Suppress-loading-of-unused-parts-of-FZ}
