#	$Id: cernman.perl,v 1.2 1998/08/19 11:53:40 goossens Exp goossens $	
# This file converts styles specific to cernman.sty styles file
# Authors of this file
# Michel Goossens   MG
# last mod. 19 August 1998 mg
$MAX_SPLIT_DEPTH = 4;	# Stop making separate files at this depth
$MAX_LINK_DEPTH = 2;	# Show no links to child nodes
$TOC_DEPTH = 4;		# Stop making separate files at this depth
...
# Fighere to be treated as figure
...
sub do_cmd_SKUIP {
    ...
    s/$next_pair_pr_rx//o;
    $params = $2;
    join ('', "<tt>CALL <B>$command</B> $params</TT><br>", $_);
}
sub do_cmd_excl {
    local ($_) = @_;
    join ('', '!', $_);
...
sub do_cmd_DEFCMD {
    ...
    s/$next_pair_pr_rx//o;
    $arg5 = $&;
    $out = sprintf ("<TT>$arg4 $arg5</TT>", $arg1, $arg2, $arg3, $arg4);
...
sub do_cmd_Shubr {
    ...
    &anchor_mylabel($anchor, "<TT>CALL $anchor $parameters<\/TT><br>");
}
sub do_cmd_Shubrz {
    local($_) = @_;
    local($anchor, $parameters);
...
sub do_cmd_Sfunc {
    ...
    $subroutine_captions{$anchor} = $CURRENT_FILE;
    &anchor_mylabel($anchor, "<TT>$all</TT><br>");
}
sub do_cmd_Sfuncii{
    local($_) = @_;
    local($anchor, $parameters);
...
sub process_cernref {
    local($ref_mark) = @_;
    ($dum,$pat) = &get_next_optional_argument; # discard optional argument
    s/$next_pair_pr_rx//o;
    local($label) = $2;
