...
\begin{enumerate}
\item
Note that on the command-line \Lit{nypatchy} the order of the ASM output
files has been rationalized and differs from the order of \Lit{ypatchy}:
\begin{verbatim}
...
\Lit{$name}, but the curly brackets are required on non-Unix machines.
The defaults for "cradle" and "print" are now standard input and output.
To get details, type \Lit{HELP} in response to the prompt from any of
the programs.

\item
File names may be given in Unix-style on the VAX; on the IBM file
names are given either as \Lit{fname.ftype.fmode} or as
\Lit{fmode/fname.ftype}.
...
\item
The specification for the control-line \Lit{+ASM} has changed substantially,
see sect. 3 and sect. 7.

\item
Control-line \Lit{+PAM} has changed a bit, see sect. 3.

\item
With the new control-line \Lit{+MORE .file-name} given in the blank deck
...
the environment variable \Lit{PLINAME} before the line is cracked for syntax.
...
It is now possible to drive by means of a \Lit{+ADD} or \Lit{+REPLACE}
...
\end{verbatim}
Delayed c/ls are marked by \Lit{"-"} in column 1,
to be converted to \Lit{"+"}.

\item
Running in update mode has now to be signalled by the new control
line \Lit{+UPDATE} in the blank deck of the cradle, see sect. 4.

\item
The title of a Pam file, that is any material before the first
line \Lit{+PATCH,...}, is now handled slightly differently: it is considered
as a patch whose name is derived from the Pam-file identifier;
see the description of \Lit{+PAM} in sect. 3.
...
significant characters in names to 8 with the new c/l \Lit{+NAMES,8.}
...
\item
The processing-mode filter for c/l \Lit{+DIVERT} has been taken away;
...
\begin{enumerate}
\item
\Lit{+DECK, dname, T=type, IF=...}

with the \Lit{T=} parameter one can signal the data-type of the contents of
...
\Lit{A*s*} means A or anything starting with AS.

It is now possible to have user-defined data types; this is described
in sect. 7.

The special T parameter \Lit{J*oin} is still used to suppress page-ejects
in listings at the start of the deck which has it.


\item
\Lit{+PATCH, pname, T=type, IF=...}

as for \Lit{+DECK}, with the extra T parameter \Lit{REP*eat} as in version 4.
...
As a result the line \Lit{+ASM} is now quite different, see sect. 7.
Nypatchy has been programmed to recognize the simpler old forms
of \Lit{+ASM}, here comes a summary, for details see sect. 7.
...
\item
\Lit{+PAM, (LABEL=)n, N=x,y, T=opt, RETURN=pname, IF=...   .file-name}
...
\end{verbatim}
The old parameter \Lit{P=pname,type} is no longer needed nor accepted:
...
the title of the ZEBRA Pam file would be \Lit{@ZEBRA}.

The typical old \Lit{+PAM} control-line like
\begin{verbatim}
...
\item
\Lit{+PARAMETER, option, N=value, IF=...}
\begin{verbatim}
...
One can now give the Pam file title directly on the \Lit{+TITLE} line:
\begin{verbatim}
   old:   +TITLE.
          PATCHY   5.00 /55  1994/02/08 11.20

   new:   +TITLE:  PATCHY   5.00 /55  1994/02/08 11.20
\end{verbatim}
Note the use of the colon \Lit{":"}.
\end{enumerate}


\section{New control-lines for version 5}

\renewcommand{\labelenumi}{4.\theenumi)\ }

\begin{enumerate}
\item
\Lit{+NAME, length, nslots, nbytes}
...
\end{verbatim}
Giving \Lit{ +NAMES,8.} one gets back-compatibility in the handling
of long names.


\item
\Lit{+GAP, gc, gl, maxl, chpl}
...
the cradle. The old options \Lit{KEEPORD, SEQORD, YESIF} are no longer needed
nor accepted.
Delayed control-lines, marked by \Lit{"-"} instead of a \Lit{"+"},
will be inserted and upgraded. Wether a Pam file contains correction
material or is a Pam file to be updated is signalled by the \Lit{T=UPDATE}
parameter on the c/l \Lit{+PAM} as before.

\item
\Lit{+SKIP, IF=... } is the opposite of \Lit{+SELF, IF=...},
that is the material following is ignored if the condition is "true".
In particular \Lit{ +SKIP. } is an unconditional skip.


\item
\Lit{+_IF, ...} et al.  to mark conditional material:
\begin{verbatim}
...
\item
\Lit{+NIL. } is a comment line, also a "soft" control-line.

For many comment lines in succession it is better to head them with
\Lit{+SKIP.} and terminate with \Lit{+SELF.} or some other "hard" c/l.

\item
\Lit{+SHOW, ASM}.

This will display the current state of the ASM handling control structure.
This may help if one is in doubt what Patchy did of one's \Lit{+ASM} lines.
...
\item
\Lit{+XDIVERT, pname [,D=dname].}

Extra diversion is like the old diversion triggered by \Lit{+DIVERT} at
...
\section{Handling of include files}
...
The new sequences \Lit{+QFxxxx} get components of the Pam file header
...
escape symbol (here and by default \Lit{"?"}), followed by the text
"txe" (if any).
\begin{verbatim}
...
\end{verbatim}
\item
In the same spirit, the new sequences \Lit{QDATE} and \Lit{QTIME} give
...
\item
The new sequence \Lit{QENVIR} calls for substitution of an environment
...
\end{verbatim}
The generic logical stream \Lit{LATEX} (or \Lit{LATEX:0}) designates all
...
4 forms of the control-line \Lit{+ASM} (using \Lit{FORT:0} as example):
\begin{verbatim}
...
\end{verbatim}
These examples each establish the physical stream \Lit{FORT}. In the
...
The generic logical streams of any of \Lit{FORT}, \Lit{CC}, \Lit{AS},
or \Lit{DATA} and the
diverted streams \Lit{FORT:2}, \Lit{CC:2}, \Lit{AS:2}, and \Lit{DATA:2},
can also be made
...
otherwise with a \Lit{+ASM} line in the cradle, \Lit{T=ATTACH} is then assumed
for such a stream.

For any other physical stream a \Lit{+ASM} line has to be used, for example:
\begin{verbatim}
...
\end{verbatim}
This sends all logical streams of \Lit{LATEX} and \Lit{TEX} together
through the physical stream \Lit{TEX} (or \Lit{TEX:0}, which is the
...
and sends it to file \Lit{fidiv}; the output from all other streams goes
to file \Lit{finorm}.
\begin{verbatim}
      +ASM, DATA,   T=BYPASS.
      +ASM, FORT,   T=ATTACH  .finorm
...
\end{verbatim}
which establishes the physical stream \Lit{TEX:2} and binds the logical
stream \Lit{LATEX:2} to it.

To specify binding without at the same time establishing a physical stream
one has to give the parameter \Lit{T=BIND}; thus to achieve the binding-only
...
\end{verbatim}
binds the data type \Lit{LATEX} to the data type \Lit{TEX}, such that
the logical streams \Lit{LATEX:i} are bound to \Lit{TEX:i}.

"Aliasing" is similar to binding, for example
\begin{verbatim}
      +ASM, TEX, LATEX, T=ALIAS.
\end{verbatim}
signals that \Lit{LATEX} is just another name for \Lit{TEX}.
...
\end{verbatim}
which tells Nypatchy that \Lit{FORT77} and \Lit{FORT99} are just other
names for the data type \Lit{FORT}.

\item
Deck splitting with \Lit{T=SPLIT}
...
\end{verbatim}
requests splitting of all decks arriving on the physical stream \Lit{FORT},
...
\Lit{FORT}, \Lit{CC},
\Lit{INCL}, and \Lit{AS}, on the machine one is running on.
For data type \Lit{CRAD} the
extension used is .cra; for \Lit{SHELL} .sh is used on UNIX, .com on VAX.
For data type \Lit{DATA} .dat is used on the VAX, and nothing on UNIX.

One may want to change the extensions used, or for the user data types or
for type \Lit{DATA} one may want to set the extension; this can be done with,
for example:
\begin{verbatim}
      +ASM, FORT, T=EXT   ..ftn
\end{verbatim}
...
if there is a deck \Lit{M432} of type \Lit{LATEX} on the Pam file, which is
processed with
\begin{verbatim}
      +ASM, TEX, LATEX, T=SPLIT, LOG  ...
\end{verbatim}
there will be an entry \Lit{"latex:1 m432"} on the log file, but
if the processing is done with
\begin{verbatim}
      +ASM, TEX, T=SPLIT, LOG  ...
      +ASM, TEX, LATEX, T=ALIAS.
\end{verbatim}
the entry will be \Lit{"tex:1 m432"}.

\item
Deck splitting with \Lit{T=MODIFY}

Giving \Lit{T=MODIFY} instead of \Lit{T=SPLIT} will cause Nypatchy to
...
As for \Lit{T=SPLIT}, one may give, for example:
\begin{verbatim}
...
Routine header lines are not generated for processing in \Lit{T=SPLIT} mode.

Note that \Lit{+ASM, T=RH...} lines merely modify the information for
...
\Lit{DATA} into a \Lit{+} if
the key of the line is that of a Patchy control line.

The data types \Lit{SHELL} and \Lit{CRAD} are initialized with
\Lit{T=CCHSUB .&+} which can be over-ridden with a \Lit{+ASM} line.
Again, this could be switched off
with \Lit{+ASM, SHELL, T=CCHSUB.} for example.
...
\end{verbatim}
This tells Patchy that material driven to the logical stream \Lit{DATA:2},
"diverted DATA", consists really of include files for C. This material
should have had a \Lit{T=INCL} declaration on the Pam file and it will be
treated as if it had: it will be sent to output through the logical
stream \Lit{INCL:2}, with the right properties, like file extension or
...
stream is bound be default to \Lit{CC:2}, and if this latter is made
...
some \Lit{+ASM} line in the blank deck of the cradle.

If no \Lit{+ASM} line for such a user data type is needed for any of the above
...
for this data type, which it then binds to the stream \Lit{DATA}.
...
\end{verbatim}
   Specify file-name prefix for \Lit{SPLIT} or \Lit{MODIFY} mode:
\begin{verbatim}
         +ASM, phys, T=PREFIX   .prefix
\end{verbatim}
   Specify log file for \Lit{SPLIT} or \Lit{MODIFY} mode:
\begin{verbatim}
...
If Nypatchy is presented with a \Lit{+ASM} line for the old Ypatchy
...
\begin{verbatim}
      ...
      +QUIT.
\end{verbatim}
The line \Lit{+ASM,23,T=ATT ...} is translated into \Lit{+ASM,DATA,T=ATTACH ...}
The line \Lit{+ASM,24.} is translated to \Lit{+ASM,CC,T=BIND.} and has hence
...
a physical stream "n" a control-line  \Lit{+ASM},n  has to be given as well
...
\Lit{P=PY_VS5} is not USE selected.
...
\begin{verbatim}
          ...
          fcasplit flibc.inc
\end{verbatim}
\Lit{P=PY_VS5} is internally pre-selected as USEd if Patchy 5 is running.
\end{enumerate}

\section{Patchy 5 on the IBM with VM/CMS}
...
File names on command lines, or on control-lines \Lit{+PAM} or \Lit{+ASM} with
the \Lit{T=ATTACH} option, are given in Unix style, for example:
\begin{verbatim}
...
for example \Lit{nylist help}. To get help about the auxiliary
programs one can give the H option, for example \Lit{nylist - h}.
...
data-type is \Lit{FORT} (IBM only).


\section{The new Patchy auxiliary Nydiff}
...
\Lit{+USE, T=INH}, patches or decks on "new" not found on "old" are
added with \Lit{+ADD}. For related decks the differences of content
are recorded with \Lit{+ADD}, \Lit{+REPLACE}, or \Lit{+DELETE}.
...
If one has the reference Pam file \Lit{old.car}, say, and one receives
the result from Nydiff in the form of the two cradles \Lit{diff.ucra}
and \Lit{diff.uexe}, one can make \Lit{new.car} with this shell script:
\begin{verbatim}
   nypatchy  -  new.car  .go  <</
   +UPDATE.
...
Pam file; all corrections are headed with \Lit{+PATCH, UPD.}
...
established with \Lit{+ASM,...,T=MODIFY,LOG} the re-compilation
of unchanged
...
\begin{enumerate}
\item
Deck splitting having been done with \Lit{T=SPLIT}
...
\item
Deck splitting having been done with \Lit{T=MODIFY}
...
\Lit{+INCLUDE} files, if any; something like:
\begin{verbatim}
      fort:1    chdirf.f
      cc:1      chdiri.c
      fort:1    getenvf.f  same
...
In \Lit{MODIFY} mode we can avoid re-compiling routines which have not changed.
...
\end{verbatim}
On the VAX  \Lit{.f} is \Lit{.FOR}, \Lit{.s} is \Lit{.MAR},
\Lit{.o} is \Lit{.OBJ} and \Lit{.a} is \Lit{.OLB}.
...
it may be declared to be empty by giving \Lit{EOF} as file name; or it may be
...
whilst "fo:i" gives the extra options to be used for stream \Lit{FORT:i}.
...
The value \Lit{"=:1"} means: same as stream 1.
...
Note that Nyshell itself and the resulting shell-script \Lit{source.shfca}
...
name would be \Lit{source.xqtlog}.
...
Here we use \Lit{+XDIVERT} to make two streams into 2 separate subdirectories.
\begin{verbatim}
#!/bin/csh -f
...
With these commands in the login file \Lit{login.com} to switch between
VAX and Alpha:
\begin{verbatim}
