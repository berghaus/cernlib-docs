...
<xsl:output 
	...
	doctype-system="/opt/sgml/dtds/jrefentry.dtd"/>
...
<xsl:template match="*">
<xsl:variable name="block-element" select="name(.) = 'para' or name(.) = 'sidebar' or name(.) = 'variablelist' or name(.) = 'itemizedlist'"/>
<xsl:if test="$block-element = 1">
	<xsl:text>&#10;</xsl:text>
</xsl:if>
