...
xtnew.sh ho.xml makehtml.xsl a.html
sed -e 's/!PERCENT!/%/g' < a.html > hobody.html
rm a.html
...
cat HOBEGIN.HTML > homaster.html
...
# A main file homaster.html will contain top level pointers
# to the various chapters. 

gawk ' BEGIN	{ while (getline < "temp.tmp" > 0 ) 
 		  { ref[$1] = $2 }
		  nout = 0
                  masterfile="homaster.html"
                  outfile=masterfile
...
rm -f a
cat  homaster.html HOEND.HTML >> a
date=`date | gawk '{print $3 "-" $2 "-" $6}'`
sed -e "s/!!DATE!!/$date/g" < a > ho.html
rm -i hobody.html homaster.html

echo "Now you should copy all these HTML files to "
echo "/afs/.cern.ch/project/cndoc/wwwasd/lhc++/HepODBMS/user-guide/"
