...
<xsl:template name="makeheading">
  <xsl:choose>
    <xsl:when test="from-ancestors(appendix)">
  ...
  <xsl:choose>
    <xsl:when test="from-ancestors(section)">
...
<xsl:template name="sectionhead">
  <xsl:param-variable name="level">1</xsl:param-variable>
...
<xsl:template match="subparagraph/stitle">
  <xsl:call-template name="sectionhead">
    <xsl:param name="level">6</xsl:param>
  </xsl:call-template>
</xsl:template>
...
<!-- <P><OBJECT ID="{from-ancestors(figure)/@id}" DATA="{@file}.gif"
   TYPE="image/gif" /></P> -->
<P CLASS="CENTERP"><A NAME="{from-ancestors(figure)/@id}"><IMG SRC="{@file}.gif"/></A></P>
</xsl:template>
<!-- url -->
